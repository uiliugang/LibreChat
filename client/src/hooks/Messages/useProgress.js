...
export default function useProgress(initialProgress = 0.01, increment = 0.007, fileSize) {
    const calculateIncrement = useCallback((size) => {
        const baseRate = 0.05;
        const minRate = 0.002;
    ...
    const incrementValue = useMemo(() => calculateIncrement(fileSize), [fileSize, calculateIncrement]);
    const [progress, setProgress] = useState(initialProgress);
    const getDynamicIncrement = useCallback((currentProgress) => {
        if (!fileSize) {
